<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.peijianName" type="text" placeholder="配件名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in peijianTypesList" :key="index"
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;">
			...
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">配件：{{item.peijianValue}}</text>
			</view>
			<!--<view class="u-box" style="color: #8799A3;">-->
			<!--</view>-->
		</view>
        <view style="float: right;">
            <text v-if="isAuth('peijian','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(item.id)"></text>
            <text v-if="isAuth('peijian','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('peijian','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 配件类型 字段 字典表查询方法
		  	let peijianTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'peijian_types'
		  	});
		  	this.peijianTypesList = peijianTypes.data.list;
		...
		methods: {
				...
				let params = {
                    ...
                    ,peijianDelete: 1
							...
							if (this.categoryName != 0) {
                                params.peijianTypes = this.categoryName
                            }

                let res = await this.$api.page(`peijian`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('peijian', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.peijianUuidNumber) {
                    searchForm['peijianUuidNumber'] = this.searchForm.peijianUuidNumber
                }
				if (this.searchForm.peijianName) {
                    searchForm['peijianName'] = this.searchForm.peijianName
                }
				if (this.searchForm.peijianName) {
					searchForm['peijianName'] = this.searchForm.peijianName
				}

                let res = await this.$api.page(`peijian`, searchForm);
				// 如果是第一页数据置空
