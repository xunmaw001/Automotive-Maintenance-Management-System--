    ...
    <view class="content">
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"#06A9FF","borderRadius":"0px","color":"#06A9FF","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}'
                        v-model='ruleForm.weixiuxiangmuLiuyanText' placeholder="评论内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`weixiuxiangmuLiuyan`, this.ruleForm.id);
        ...
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.weixiuxiangmuLiuyanText)) {
                this.$utils.msg(`评论内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`weixiuxiangmuLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`weixiuxiangmuLiuyan`, this.ruleForm);
                }
