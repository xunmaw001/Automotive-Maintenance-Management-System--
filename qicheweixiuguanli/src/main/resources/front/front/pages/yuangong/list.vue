<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.yuangongName" type="text" placeholder="员工姓名"></input>
		</view>
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.yuangongName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.yuangongPhoto?product.yuangongPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.yuangongNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.yuangongNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('yuangong','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('yuangong','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('yuangong','新增')" class="add-btn" @click="onAddTap()">新增</button>
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	this.sexTypesList = sexTypes.data.list;
	   		//当前表的 身份 字段 字典表查询方法
		  	let shenfenTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'shenfen_types'
		  	});
		  	this.shenfenTypesList = shenfenTypes.data.list;
