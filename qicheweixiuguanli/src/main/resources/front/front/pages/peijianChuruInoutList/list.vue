<template>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;">
			<view style="font-size: 30rpx;">
				<b>{{item.peijianChuruInoutListId}}</b>
			</view>
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">出入库详情：{{item.peijianChuruInoutListValue}}</text>
			</view>
			<!--<view class="u-box" style="color: #8799A3;">-->
			<!--</view>-->
		</view>
        <view style="float: right;">
            <text v-if="isAuth('peijianChuruInoutList','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(item.id)"></text>
            <text v-if="isAuth('peijianChuruInoutList','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('peijianChuruInoutList','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.peijian_churu_inout_listName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`peijianChuruInoutList`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('peijianChuruInoutList', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
                ...
                let res = await this.$api.page(`peijianChuruInoutList`, searchForm);
				// 如果是第一页数据置空
